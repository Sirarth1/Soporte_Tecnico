<?php
    ...
    $dap = $_REQUEST['dapsis'];
    $ip = $_REQUEST['IP'];
    $reg_general = $conection->prepare("INSERT INTO infogeneral (id_user, No_Dapsis, tipo_equipo, modelo_equipo, marca, no_serie, id_ip) 
                                    VALUES (?, ?, ?, ?, ?, ?, ?) ");
    ...
    if($reg_general->execute() == true){
        $reg_ip = $conection->prepare("UPDATE ips SET estado=1 WHERE id_ip=$ip");
        if ($reg_ip->execute() == true) {
